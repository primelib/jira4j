...
case $1 in
  "update")
    ...
    curl -o "$RESTV2_SPEC" https://dac-static.atlassian.com/cloud/jira/platform/swagger.v3.json
    jq '.info.title="Jira REST V2"' "$RESTV2_SPEC" | sponge "$RESTV2_SPEC" # overwrite name
    jq '.components.schemas.Attachment.properties.size."x-name-override"="fileSize"' "$RESTV2_SPEC" | sponge "$RESTV2_SPEC" # size is usually a reserved word on a Map
    ...
    curl -o "$RESTV3_SPEC" https://dac-static.atlassian.com/cloud/jira/platform/swagger-v3.v3.json
    jq '.info.title="Jira REST V3"' "$RESTV3_SPEC" | sponge "$RESTV3_SPEC" # overwrite name
    jq '.components.schemas.Attachment.properties.size."x-name-override"="fileSize"' "$RESTV3_SPEC" | sponge "$RESTV3_SPEC" # size is usually a reserved word on a Map
