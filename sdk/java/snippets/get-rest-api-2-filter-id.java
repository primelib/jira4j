...
public class Get2FilterByIdV1Example {
    public void execute() {
        // Maven coordinates: io.github.primelib:jira-datacenter-client:<version>
        var client = JiraDataCenterClientFactory.create();

        try {
            var response = client.filtersApi().get2FilterByIdV1(new Get2FilterByIdV1OperationSpec(spec -> {
                    // operation parameters (all available mutable fields)
                    // spec.id(/* Long */); // id: required - The ID of the filter to return.
            ...
            switch (response) {
                case Get2FilterByIdV1Response.OkResponse r -> {
                    // handle 200, r.data() contains typed payload
                }
                case Get2FilterByIdV1Response.Unknown r -> {
