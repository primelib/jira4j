...
public class Delete2IssueByIssueIdOrKeyWorklogByIdV1Example {
    public void execute() {
        // Maven coordinates: io.github.primelib:jira-datacenter-client:<version>
        var client = JiraDataCenterClientFactory.create();

        try {
            var response = client.issueWorklogsApi().delete2IssueByIssueIdOrKeyWorklogByIdV1(new Delete2IssueByIssueIdOrKeyWorklogByIdV1OperationSpec(spec -> {
                    ...
                    // spec.increaseBy(/* String */); // increaseBy: optional - The amount to increase the issue's remaining estimate by, as days (\#d), hours (\#h), or minutes (\#m or \#). For example, *2d*. Required when `adjustEstimate` is `manual`.
                    // spec.overrideEditableFlag(/* Boolean */); // overrideEditableFlag: optional - Whether the work log entry should be added to the issue even if the issue is not editable, because jira.issue.editable set to false or missing. For example, the issue is closed. Connect and Forge app users with admin permission can use this flag.
            ...
            switch (response) {
                case Delete2IssueByIssueIdOrKeyWorklogByIdV1Response.Unknown r -> {
